 ...
 */
 ...
 public class Board {
     ...
     * avoilable at the start of the game).
     */
    ...
    private void checkDiagonalFromTopRight(int x, int y) {
        if (BOARD_WIDTH -1-x == y){
            for (int i = 1; i < BOARD_WIDTH; i++) {
                if (board[BOARD_WIDTH -1-i][i] != board[BOARD_WIDTH -1-i][i-1]) {
                    break;
                }
                if (i == BOARD_WIDTH - 1) {
                    winner = playersTurn;
    ...
    public Board getDeepCopy(){
        ...
        board.movesAvailable    = this.movesAvailable;
